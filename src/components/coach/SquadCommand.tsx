...
interface Athlete {
  ...
  lastDrill?: string;
  trend?: string;
  stats?: { subject: string; A: number }[];
}

export default function SquadCommand() {
  const { t } = useTranslation();
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [squad, setSquad] = useState<Athlete[]>([]);
  ...
  useEffect(() => {
    async function fetchAthletes() {
      ...
      // Map DB data to component shape with defaults for missing fields
      const mapped: Athlete[] = (data || []).map(a => ({
        id: a.id,
        name: a.name,
        xp: a.xp ?? 0,
        role: a.role,
  ...
  const selectedAthlete = squad.find(a => a.id === selectedId);

  const getProScore = (athlete: Athlete) => {
    return Math.round(((athlete.wellness ?? 0) + (athlete.consistency ?? 0) + ((athlete.xp ?? 0) / 25)) / 3);
  };

  const sortedLeaderboard = [...squad].sort((a, b) => getProScore(b) - getProScore(a));

  const getFatigueStatus = (wellness: number = 72, load: number = 78) => {
    if (wellness < 40 && load > 75) return 'RED_ZONE';
    if (wellness < 60) return 'WARNING';
    return 'OPTIMAL';
  };

  const handleQuickAward = async (athleteId: string, name: string) => {
    // Award 100 XP in Supabase
    const athlete = squad.find(a => a.id === athleteId);
    if (!athlete) return;

    const newXp = (athlete.xp ?? 0) + 100;
    ...
    if (!error) {
      setSquad(prev => prev.map(a => a.id === athleteId ? { ...a, xp: newXp } : a));
      setLastAwarded(name);
  ...
  const awardMerit = async (e: React.MouseEvent | React.PointerEvent, athleteId: string, name: string) => {
    if (e) e.stopPropagation();
    await handleQuickAward(athleteId, name);
  ...
  if (loadingSquad) return (
    <div className="min-h-[400px] flex items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Loading Squad...</p>
      </div>
    </div>
  );

  // --- EMPTY STATE ---
  if (!loadingSquad && squad.length === 0) return (
    <div className="min-h-[400px] flex items-center justify-center">
      <div className="text-center space-y-4 bg-slate-900 border border-slate-800 p-12 rounded-[3rem]">
        <Trophy size={40} className="text-slate-600 mx-auto" />
        <h3 className="text-white font-black uppercase italic text-xl">No Athletes Yet</h3>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">
          Athletes will appear here once they sign up with the athlete role.
        </p>
      </div>
    </div>
  );

  const avgXp = squad.length > 0 
    ? Math.round(squad.reduce((sum, a) => sum + (a.xp ?? 0), 0) / squad.length)
    : 0;

  const atRiskCount = squad.filter(a => 
    getFatigueStatus(a.wellness, a.load) === 'RED_ZONE'
  ).length;
      ...
      {!selectedId && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-3xl flex flex-col justify-center text-start">
            ...
            <span className="text-xl font-black italic text-white">{avgXp.toLocaleString()}</span>
          </div>
          <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-3xl flex flex-col justify-center text-start">
            ...
            <span className="text-xl font-black italic text-white leading-none">{atRiskCount} {t('squad_page.at_risk_count')}</span>
          </div>
        </div>
      )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="text-[8px] font-black text-orange-500">{athlete.xp} XP</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 text-start">
                      ...
                      <p className="text-[11px] font-bold text-white italic leading-relaxed">
                        Level {calculateLevel(selectedAthlete.xp)} — {selectedAthlete.xp} XP total.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-4">
                      <button onClick={(e) => awardMerit(e, selectedAthlete.id, selectedAthlete.name)} className="bg-slate-800 hover:bg-blue-600 px-6 rounded-2xl transition-all border border-slate-700 text-white py-4 text-[10px] font-black uppercase">🏆 {t('squad_page.award_btn')}</button>
                ...
                <div key={athlete.id} className={`flex items-center gap-6 p-5 rounded-2xl border transition-all ${index === 0 ? 'bg-amber-500/5 border-amber-500/30' : 'bg-slate-950 border-slate-800'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="w-full h-1 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-500 transition-all duration-1000" style={{ width: `${proScore}%` }}></div>
                    </div>
                  </div>
                  <div className="text-orange-500"><Flame size={16} fill="currentColor" className={athlete.consistency && athlete.consistency > 95 ? 'animate-bounce' : ''} /></div>
                </div>
              );
            })}
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950 z-[100] flex flex-col p-12 animate-in fade-in zoom-in duration-500 overflow-y-auto">
          ...
          <div className="flex-1 flex flex-col items-center max-w-5xl mx-auto w-full py-10">
            <div className="text-center mb-16">
              <Medal size={64} className="text-amber-500 mx-auto mb-6" />
              <h2 className="text-5xl font-black italic uppercase text-white tracking-tighter mb-4">Sousse Elite Standings</h2>
              <div className="h-1 w-32 bg-amber-500 mx-auto rounded-full shadow-[0_0_20px_rgba(245,158,11,0.6)]"></div>
            </div>
                ...
                return (
                  <div key={athlete.id} className={`flex items-center gap-8 p-8 rounded-[2.5rem] border-2 transition-all ${index === 0 ? 'bg-amber-500/10 border-amber-500 scale-105 shadow-2xl shadow-amber-500/10' : 'bg-slate-900 border-slate-800'}`}>
                    ...
                    <div className="flex-1 text-start">
                      <h4 className="text-2xl font-black uppercase text-white mb-2">{athlete.name}</h4>
                      <div className="w-full h-3 bg-slate-950 rounded-full overflow-hidden">
                        <div className="h-full bg-amber-500" style={{ width: `${proScore}%` }}></div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-black italic text-amber-500">{proScore}%</div>
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Professionalism</div>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-[110] flex items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
          <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl">
            ...
            <div className="text-center mb-10">
              <h3 className="text-3xl font-black italic uppercase text-white tracking-tighter">{t('quick_award.title')}</h3>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('quick_award.subtitle')}</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {squad.map(a => (
                <button key={a.id} onClick={() => handleQuickAward(a.id, a.name)} className="bg-slate-950 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-3 hover:border-blue-500 transition-all group active:scale-95">
                  ...
                  <span className="text-[9px] font-black text-orange-500">{a.xp} XP</span>
                </button>
              ))}
            </div>
            {lastAwarded && (
              <div className="mt-8 bg-green-500/10 border border-green-500/20 p-4 rounded-2xl flex items-center justify-center gap-3 animate-in slide-in-from-top-4">
                <Star className="text-green-500 fill-green-500" size={16} />
                <span className="text-[10px] font-black uppercase text-green-500 tracking-widest">{t('quick_award.success')} to {lastAwarded}! +100 XP</span>
              </div>
            )}
          </div>
        </div>
